...
DEAL_DIR   = Path(r"D:\aaa_my_iwhalecloud\VScode_AI\HunyuanOCR\津巴布韦API开发测试\WAY2\金属ID-MAIN-INFO-1-DEAL-OUT")
...
def _merge_results(heuristic: dict, structured: dict | None) -> dict:
    ...
    for field in _FIELDS:
        ...
        if field == "gender":
            # 规则结果更可靠（直接匹配 CIT M/F），仅规则为空时用结构化
...
def process_single_image(image_path: Path) -> tuple[dict, dict]:
    ...
    # 初始化来源为 heuristic（如果字段非空）否则为 none
    sources = {}
    for k in _FIELDS:
    ...
    if _needs_structured_fallback(result):
        try:
            ...
            if structured:
                ...
                # 更新来源：若原来规则为空，且合并后有了值，则标记为 structured
                for k in _FIELDS:
...
def main():
    ...
    print("字段提取来源统计")
    print("=" * 60)
    total_processed = done - skipped  # 实际处理的图片数（不包含失败）
