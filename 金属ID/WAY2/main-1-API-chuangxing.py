...
# ── OCR 调用（通过 HunyuanOCR 模型） ──────────────────────
def _call_ocr(image_bytes: bytes, mime: str) -> str:
    """将图片发送给 HunyuanOCR，返回坐标文本"""
    ...
    for attempt in range(MAX_RETRIES + 1):
        try:
            ...
            if attempt < MAX_RETRIES:
                ...
                raise RuntimeError(f"结构化提取调用失败: {str(e)}")
...
async def extract_id_fields(file: UploadFile = File(...)):
    ...
    try:
        result = extract_from_image(contents)
    except Exception as e:
