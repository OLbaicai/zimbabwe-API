...
# ── 辅助函数（完全不变，此处省略，实际代码需保留原有全部函数） ──
# 请务必把下面函数完整粘贴过来：
# _extract_segments, _build_ocr_summary, _extract_id_from_ocr,
# _extract_gender_from_ocr, _extract_date_from_ocr, _extract_names_from_ocr,
# _needs_structured_fallback, _extract_json_payload, _merge_results
...
# ── FastAPI 应用 ──────────────────────────────────────
app = FastAPI(title="Zimbabwe ID Card Extractor")
...
async def extract_id_fields(file: UploadFile = File(...)):
    ...
    try:
        result = await loop.run_in_executor(executor, extract_from_image, contents)
