...
# 识别结果输出目录（不存在会自动创建）
OUTPUT_DIR = Path(r"D:\aaa_my_iwhalecloud\VScode_AI\HunyuanOCR\津巴布韦API开发测试\身份证ID-金属\金属ID-OUT")
...
【字段1：id_number】
一般由8位数字+1位字母（大写字母）+2位数字组成，例如：02033711E02
一共输出11位或者12位字符，不要输出少或者多。
此字段图片中存在空格，识别时请接连数字不要舍去数字或者字母。不要输出CIT三个字母。

【字段2：surname】
位置在id_number下方，通常是大写字母，表示姓氏

【字段3：first_name】
位置在surname下方，通常是大写字母，表示名字

【字段4：birth_date】
位置在first_name下方，通常是DD.MM.YY  DD.MM.YYYY  或者是D/M/YY等格式的日期，表示出生日期

【字段5：gender】
位置在id_number右侧，通常是大写字母，表示性别，例如：M或F

【重要约束】
不要输出中文，输出只能是数字或者字母，不要包含任何其他符号如-或者空格。
