...
对比预测 JSON txt 与真实标签 JSON txt，输出：
  - 汇总报告 (.txt)
  - 全部错误样本明细 (.csv)
"""
...
# 配置（修改这里即可）
# ══════════════════════════════════════════════════════════════
PRED_DIR   = Path(r"D:\aaa_my_iwhalecloud\VScode_AI\HunyuanOCR\津巴布韦API开发测试\津巴布韦-彩色驾照\txt\MAIN-4-OUT")
LABEL_DIR  = Path(r"D:\aaa_my_iwhalecloud\VScode_AI\HunyuanOCR\津巴布韦API开发测试\津巴布韦-彩色驾照\驾照标注-JSON")
REPORT_DIR = Path(r"D:\aaa_my_iwhalecloud\VScode_AI\HunyuanOCR\津巴布韦API开发测试\津巴布韦-彩色驾照\report")

# 输出文件名
REPORT_TXT = "MAIN-4-eval_pred_vs_label_json.txt"
ERROR_CSV  = "MAIN-4-errors.csv"
# ══════════════════════════════════════════════════════════════

FIELDS = ("id_number", "surname", "first_name", "birth_date")
...
def _norm_id_number(value: Any) -> str:
    """归一化 ID 号码：只保留字母和数字，忽略所有分隔符。"""
    s = _as_str(value)
    # 移除一切非字母数字的字符（空格、横线、斜杠等）
...
def evaluate_json_dirs(
    ...
    for label_path in label_files:
        ...
        if not pred_path.exists():
            ...
            pred_obj = _try_parse_json(_read_text(pred_path))
            if pred_obj is None:
                parse_err_pred += 1
