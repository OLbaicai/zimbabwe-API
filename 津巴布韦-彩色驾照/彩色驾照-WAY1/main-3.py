...
# 识别结果输出目录（不存在会自动创建）
OUTPUT_DIR = Path(r"D:\aaa_my_iwhalecloud\VScode_AI\HunyuanOCR\津巴布韦API开发测试\津巴布韦-彩色驾照\txt\MAIN-3-OUT")
...
def _fix_id_if_no_letter(raw_id: str) -> str:
    """
    若 id_number 去除分隔符后没有任何字母，则根据长度：
    - 13位：将第11位（索引10）替换为字母
    - 14位：将第12位（索引11）替换为字母
    使用用户提供的数字→字母映射（后者覆盖前者）：
    2→Z, 1→T, 8→B, 0→D, 1→J, 5→S, 7→T, 1→L, 6→G
    由于 1 出现多次，最终映射为 1→L（最后一条），但这里我们改为按顺序优先匹配，
    即如果数字在映射表中就替换，重复的以后出现的为准。
    为安全起见，我们使用字典顺序，如果有重复键，后面的值会覆盖前面的值。
    最终映射：0→D, 1→L, 2→Z, 5→S, 6→G, 7→T, 8→B
    """
    # 去除分隔符，只保留字母数字
    ...
    # 映射表（用户指定的顺序，1→L 覆盖了前面的 1→T 和 1→J）
    digit_map = {
        '0': 'D',
        '1': 'L',   # 最终用 L，如果你希望优先 T 或 J，可调整顺序，比如 '1': 'T'
        '2': 'Z',
        '5': 'S',
        '6': 'G',
        '7': 'T',
...
def postprocess_result(raw_text: str) -> str:
    """对模型返回的JSON进行后处理：姓名去变音，ID补字母（仅当全无字母时）。"""
    obj = _try_parse_json(raw_text)
    if obj is None:
        return raw_text   # 解析失败，保持原样
